...
// محاكاة بسيطة لدورة حياة الطلب في الوضع التجريبي (بدون Supabase) — نفس فكرة
// apps/web/src/lib/demoStore.ts بس بتخزين في الذاكرة بس (مش AsyncStorage) لأن الهدف
// هنا معاينة سريعة للتطبيق، مش جلسة تفضل محفوظة بعد إعادة تشغيل التطبيق.
...
function toOrder(stored: StoredDemoOrder): Order {
  const status = computeStatus(stored)
  return {
    id: stored.id,
    ...
    courierId: status === 'out_for_delivery' || status === 'delivered' ? 'demo-courier' : null,
    notes: null,
